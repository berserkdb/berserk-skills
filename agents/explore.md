...
**Resource / attribute access:** Berserk's KQL resolves OTel dotted keys permissively — write the plain dotted form: `resource.service.name`, `resource.k8s.pod.name`, `attributes.http.method`. The equivalent `resource['service.name']` bracket form also resolves and is only needed when a literal key actually conflicts with a sibling object (rare; this doesn't happen in our data). `fieldstats` / `otel-log-stats` emit `AttributePath` as dotted paths you can paste straight into queries.
**Column naming tip:** `summarize by tostring(resource.service.name)` auto-generates the column name `resource_service_name` (dots become underscores) — no quoting needed. Alias to keep things short: `by svc=tostring(resource.service.name)`.
